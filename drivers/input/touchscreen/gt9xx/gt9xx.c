...
static u8 bgt9118 = FALSE;
static u8 bgt9115 = FALSE;
...
extern char *displayic_get(void);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    if (bgt9118) {
		send_cfg_buf[0] = gtp_dat_gt11_8;
		cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt11_8);
    }

    if (bgt9115) {
		send_cfg_buf[0] = gtp_dat_gt11_5;
		cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt11_5);
	}
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    else
    {
		...
		if (buf[2] == '9' && buf[3] == '2' && buf[4] == '7' && buf[5] == '1') {
			bgt911 = FALSE;
			bgt9118 = FALSE;
			bgt9115 = FALSE;
			bgt970 = FALSE;
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//add by chenhaiman
	if (strcmp(displayic_get(), "JD9365D") == 0) {
		val = 9118; //8吋老触摸
	}
