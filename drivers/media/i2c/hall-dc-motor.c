...
static int motor_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_IRIS_ABSOLUTE:
		...
		duty = (u64)motor->pwm_state.period * ctrl->val;
		motor->pwm_state.duty_cycle = div_u64(duty, IRIS_MAX_LOG);
		pwm_apply_state(motor->pwm, &motor->pwm_state);
		dev_dbg(motor->dev, "iris, ctrl->val %d, pwm duty %d, period %d, polarity %d\n",
			ctrl->val,
