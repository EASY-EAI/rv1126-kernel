...
static int nvp6188_read_reg(struct i2c_client *client, u8 reg, u8 *val)
{
	...
	dev_err(&client->dev,
		"nvp6188 read reg(0x%x val:0x%x) failed !\n", reg, *val);
...
ntpal: 1:25p, 0:30p
*/
static __maybe_unused void nv6188_set_chn_960h(struct nvp6188 *nvp6188, u8 ch, u8 ntpal) {
	unsigned char val_0x54, val_20x01;
	struct i2c_client *client = nvp6188->client;
...
ntpal: 1:25p, 0:30p
*/
static __maybe_unused void nv6188_set_chn_720p(struct nvp6188 *nvp6188, u8 ch, u8 ntpal) {
	unsigned char val_0x54, val_20x01;
	struct i2c_client *client = nvp6188->client;
...
ntpal: 1:25p, 0:30p
*/
static __maybe_unused void nv6188_set_chn_1080p(struct nvp6188 *nvp6188, u8 ch, u8 ntpal) {
	unsigned char val_0x54, val_20x01;
	struct i2c_client *client = nvp6188->client;
...
static int detect_thread_function(void *data)
{
	...
	struct i2c_client *client = nvp6188->client;
	unsigned char bits = 0, ch, val_13x70, val_13x71;
	if (nvp6188->power_on) {
		nvp6188_write_reg(client, 0xff, 0x00);
...
static __maybe_unused int nvp6188_auto_detect(struct nvp6188 *nvp6188) {
	int ret = 0;
	int check_cnt = 0, ch = 0;
	unsigned char ch_vfc[4]={0xff,0xff,0xff,0xff};
	unsigned char val_13x70, val_13x71;
	struct i2c_client *client = nvp6188->client;
    ...
    ret = -1;
	while((check_cnt++) < 50) {
		dev_dbg(&client->dev, "check_cnt %d",check_cnt);
		for (ch = 0; ch < 4; ch++) {
			ch_vfc[ch] = nv6188_read_vfc(nvp6188, ch);
			if(ch_vfc[ch] != 0xff) {
				dev_dbg(&client->dev, "ch[%d] read vfc 0x%2x", ch, ch_vfc[ch]);
			}
		}
	...
	for (ch = 0; ch < 4; ch++) {
		...
		switch(ch_vfc[ch]) {
			...
			break;
			default:
				dev_err(&client->dev, "channel %d ch_vfc not valid, def 720p pal\n", ch);
				nvp6188->cur_mode->channel_reso[ch] = NVP_RESO_720P_PAL;
			break;
		}
	}
...
static int check_chip_id(struct i2c_client *client){
	...
	unsigned char chip_id, chip_revid;
	nvp6188_write_reg(client, 0xFF, 0x00);
...
int nvp6188_audio_init(struct nvp6188 *nvp6188)
{
	struct i2c_client *client = nvp6188->client;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (of_property_read_u32(node, "via_vicap", &nvp6188->via_vicap)){
		dev_err(dev, "default via vicap!\n");
		nvp6188->via_vicap = 1;
	}

	nvp6188->client = client;
	if (nvp6188->via_vicap)
		nvp6188->cur_mode = &supported_modes[0];
	else
		nvp6188->cur_mode = &supported_modes[1];
	...
	if (!of_property_read_string(node, "rockchip,audio-in-format", &str)) {
		...
		nvp6188->audio_in = kzalloc(sizeof(struct nvp6188_audio), GFP_KERNEL);
		if (!nvp6188->audio_in) {
	...
	if (!of_property_read_string(node, "rockchip,audio-out-format", &str)) {
		...
		nvp6188->audio_out = kzalloc(sizeof(struct nvp6188_audio), GFP_KERNEL);
		if (!nvp6188->audio_out) {
	...
	nvp6188->input_dev  = input_allocate_device();
	if (nvp6188->input_dev == NULL) {
		printk(KERN_ERR"failed to allocate nvp6188 input device\n");
		return -1;
	}
	...
	if (input_register_device(nvp6188->input_dev)) {
		pr_err("%s: failed to register nvp6188 input device\n", __func__);
		input_free_device(nvp6188->input_dev);
		return -1;
	}
...
static int nvp6188_remove(struct i2c_client *client)
{
	...
	input_unregister_device(nvp6188->input_dev);
	input_free_device(nvp6188->input_dev);

	if (nvp6188->audio_in)
		kfree(nvp6188->audio_in);
	if (nvp6188->audio_out)
		kfree(nvp6188->audio_out);
